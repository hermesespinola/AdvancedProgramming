...
unsigned long hash2(void* key, unsigned size) {
  char *str = (char *)key;
  unsigned hash = 5381;
  int c;
  while ((c = *str++)) {
...
static char * testInsert() {
  ...
  initHash(&myHash, size, hash, cmpKeys, copy, copy);
  insertHash(&myHash, (void *) key, (void *) value);
  muAssert("error, key data[0].elements[0]", strcmp(myHash.data[0].elements[0].key, key) == 0);
  muAssert("error, value data[0].elements[0]", strcmp(myHash.data[0].elements[0].value, value) == 0);
}
...
static char * testInit() {
  ...
  unsigned size = 20;
  initHash(&myHash, size, hash, cmpKeys, copy, copy);
  muAssert("error, hash size must be 20", myHash.size == 20);
